...
export interface WindowsSystemProvider {
  arch(): string;
  release(): string;
  hostname(): string;
  totalmem(): number;
...
export class WindowsPlatformAdapter implements PlatformAdapter {
  public readonly platformType = 'windows' as const;
  public readonly isSupported = true;

  constructor(private readonly provider: WindowsSystemProvider = os) {}
  ...
  public async isCommandAvailable(command: string): Promise<boolean> {
    try {
      const result = await executeSafeCommand('where', [command], { timeoutMs: 3000 });
